...
//
// By Yingzhi Zhou (z5125679)
//    ... (z0000000)
//
// Written on 2017-10-21
// Tutor (dayHH-lab)
...
static int findMatchingCardColor (Game game, color color) {
    int cardIndex = 0;
    while (cardIndex < handCardCount(game)) {
        if(handCard(game, cardIndex)->color == color) {
            return cardIndex;
        }
        cardIndex++;
    }
...
static int canDrawCard (Game game) {
    ...
    while (currentCard != NULL) {
        if(doCardsMatch(topCard, currentCard) == TRUE) {
            shouldDraw == FALSE;
        }
        i++;
        currentCard = handCard(game, i);
    }
